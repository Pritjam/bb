...
/*
 * submitr - Submit a client result string to the server.
 */
int submitr(char *hostname,   /* Server domain name */
            int port,         /* Server port */
            char *course,     /* Course name */
            char *userid,     /* Userid */
            char *user_pwd,   /* User password (for authentication) */
            char *lab,        /* Lab name */
            ...
            char *status_msg) /* Status message returned to caller */
{
  ...
  if (connect(clientfd, (SA *)&serveraddr, sizeof(serveraddr)) < 0) {
    strcpy(status_msg, "Error: Unable to connect to the server");
    close(clientfd);
    return -1;
  }
  ...
  req_size =
      strlen(course) + strlen(userid) + strlen(lab) + 3 * result_size + 128;
  ...
  sprintf(
      buf,
      "GET /%s/submitr.pl/?userid=%s&userpwd=%s&lab=%s&result=%s&submit=submit "
      "HTTP/1.0\r\n\r\n",
  ...
  if (rio_readlineb(&rio, buf, SUBMITR_MAXBUF) <= 0) {
    strcpy(status_msg, "Error: Client unable to read first header from server");
    close(clientfd);
    return -1;
  }
  sscanf(buf, "%s %d %[a-zA-z ]", version, &errcode, errmsg);
  if (errcode != 200) {
    sprintf(status_msg, "Error: HTTP request failed with error %d: %s", errcode,
            errmsg);
    close(clientfd);
    return -1;
  }
  ...
  while (strcmp(buf, "\r\n")) {
    if (rio_readlineb(&rio, buf, SUBMITR_MAXBUF) <= 0) {
      strcpy(status_msg, "Error: Client unable to read headers from server");
      close(clientfd);
      return -1;
    }
  }
...
int init_driver(char *status_msg) {
  ...
  /* Ignore any terminating SIGPIPE or SIGPOLL signals */
  ...
  /* Establish a connection with the server */
  if (connect(clientfd, (SA *)&serveraddr, sizeof(serveraddr)) < 0) {
    sprintf(status_msg, "Error: Unable to connect to server %s", hostname);
    close(clientfd);
    return -1;
  }
 ...
 *     it needs to transmit an autoresult string to the server
 */
int driver_post(char *userid, char *user_pwd, char *result, int autograded,
                char *status_msg) {
  int status;
   ...
   * the autoresult string to the server
   */
  if (userid && strcmp(userid, "")) {
    status = submitr(SERVER_NAME, SERVER_PORT, COURSE_NAME, userid, user_pwd,
                     LABID, result, status_msg);
    return status;
  }
